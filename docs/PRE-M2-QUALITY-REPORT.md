...
> 문서화 브랜치: `refactor/pre-m2-quality-baseline`
...
이 문서는 M2 REST API 설계 전에 확인된 Validation, Security, JPA와 Pagination 부채를 분석한다. 완성된 구현 코드나 적용 결과를 포함하지 않으며, PR #99의 최종 코드·테스트·View·설정 상태는 `master`와 동일하다.
